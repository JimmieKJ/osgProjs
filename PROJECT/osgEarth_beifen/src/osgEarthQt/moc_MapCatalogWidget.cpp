/****************************************************************************
** Meta object code from reading C++ file 'MapCatalogWidget'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "D:/PROJECT/osgEarth/src/osgEarthQt/MapCatalogWidget"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'MapCatalogWidget' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_osgEarth__QtGui__MapCatalogWidget_t {
    QByteArrayData data[10];
    char stringdata0[158];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_osgEarth__QtGui__MapCatalogWidget_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_osgEarth__QtGui__MapCatalogWidget_t qt_meta_stringdata_osgEarth__QtGui__MapCatalogWidget = {
    {
QT_MOC_LITERAL(0, 0, 33), // "osgEarth::QtGui::MapCatalogWi..."
QT_MOC_LITERAL(1, 34, 12), // "onMapChanged"
QT_MOC_LITERAL(2, 47, 0), // ""
QT_MOC_LITERAL(3, 48, 18), // "onSelectionChanged"
QT_MOC_LITERAL(4, 67, 23), // "onTreeItemDoubleClicked"
QT_MOC_LITERAL(5, 91, 16), // "QTreeWidgetItem*"
QT_MOC_LITERAL(6, 108, 4), // "item"
QT_MOC_LITERAL(7, 113, 3), // "col"
QT_MOC_LITERAL(8, 117, 17), // "onTreeItemChanged"
QT_MOC_LITERAL(9, 135, 22) // "onTreeSelectionChanged"

    },
    "osgEarth::QtGui::MapCatalogWidget\0"
    "onMapChanged\0\0onSelectionChanged\0"
    "onTreeItemDoubleClicked\0QTreeWidgetItem*\0"
    "item\0col\0onTreeItemChanged\0"
    "onTreeSelectionChanged"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_osgEarth__QtGui__MapCatalogWidget[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   39,    2, 0x08 /* Private */,
       3,    0,   40,    2, 0x08 /* Private */,
       4,    2,   41,    2, 0x08 /* Private */,
       8,    2,   46,    2, 0x08 /* Private */,
       9,    0,   51,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, 0x80000000 | 5, QMetaType::Int,    6,    7,
    QMetaType::Void, 0x80000000 | 5, QMetaType::Int,    6,    7,
    QMetaType::Void,

       0        // eod
};

void osgEarth::QtGui::MapCatalogWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        MapCatalogWidget *_t = static_cast<MapCatalogWidget *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->onMapChanged(); break;
        case 1: _t->onSelectionChanged(); break;
        case 2: _t->onTreeItemDoubleClicked((*reinterpret_cast< QTreeWidgetItem*(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 3: _t->onTreeItemChanged((*reinterpret_cast< QTreeWidgetItem*(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 4: _t->onTreeSelectionChanged(); break;
        default: ;
        }
    }
}

const QMetaObject osgEarth::QtGui::MapCatalogWidget::staticMetaObject = {
    { &QFrame::staticMetaObject, qt_meta_stringdata_osgEarth__QtGui__MapCatalogWidget.data,
      qt_meta_data_osgEarth__QtGui__MapCatalogWidget,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *osgEarth::QtGui::MapCatalogWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *osgEarth::QtGui::MapCatalogWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_osgEarth__QtGui__MapCatalogWidget.stringdata0))
        return static_cast<void*>(const_cast< MapCatalogWidget*>(this));
    return QFrame::qt_metacast(_clname);
}

int osgEarth::QtGui::MapCatalogWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QFrame::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
