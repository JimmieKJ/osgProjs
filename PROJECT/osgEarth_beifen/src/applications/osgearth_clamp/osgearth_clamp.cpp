...
public:
    ...
    {    
		std::cout<<"1   "<<tileKey.getLevelOfDetail()<<" "<<tileKey.getTileX()<<" "<<tileKey.getTileY()<<std::endl;
        if ((int)tileKey.getLevelOfDetail() > _minLevel && _maxLevel < (int)tileKey.getLevelOfDetail())
        {
			...
			std::cout<<"2   "<<tileKey.getLevelOfDetail()<<" "<<tileKey.getTileX()<<" "<<tileKey.getTileY()<<std::endl;
...
main(int argc, char** argv)
{
    ...
    osg::Node* tree = osgDB::readNodeFile("D:/PROJECT/OpenSceneGraph-Data/cow.osg");         
